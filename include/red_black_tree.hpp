...
namespace ft{
    ...
    struct Node
    {
        ...
        Node    &operator=(const Node<value_type> &rhs)
        {
        ...
        Node    (const Node<value_type> &rhs)
        {
            this->value = rhs.value;
            this->parent = rhs.parent;
            this->left = rhs.left;
        ...
        Node(value_type *void_val = 0) : value(void_val), parent(NULL), left(NULL),  right(NULL),colour(0), is_nil(0) {}

    };


    template <typename value_type, typename key_type, typename Compare = ft::less<value_type>, class Allocator = std::allocator<value_type> >
        class rb_tree
        {
            public:
            ...
            rb_tree() 
            {
                ...
                _alloc.construct(null_node, Node<value_type>());
                root = null_node;
                null_node->value = val_alloc.allocate(1);
                val_alloc.construct(null_node->value, value_type());
                null_node->left = NULL;
            ...
            rb_tree(const Compare comp, const Allocator alloc) : val_alloc(alloc), _alloc(_alloc()), _comp(comp), _size(0)
            {
                std::allocator<value_type> val_alloc;
                ...
                root = null_node;
                null_node->value = val_alloc.allocate(1);
                val_alloc.construct(null_node->value, Node<value_type>());
                null_node->left = NULL;
                null_node->right = NULL ;
                null_node->parent = NULL;
                null_node->colour = BLACK ;
            ...
            rb_tree(const  Self &rhs) :  val_alloc(rhs.val_alloc),_alloc(rhs._alloc), _comp(rhs._comp), _size(0)
            {
                std::allocator<value_type> val_alloc;
               null_node = _alloc.allocate(1);
                 val_alloc.construct(null_node->value, value_type());
            ...
            ~rb_tree()
            {
                ...
                val_alloc.deallocate(null_node->value, 1);
            ...
            NodePtr next(NodePtr node) const
        {
            ...
            if (current->is_nil  || current == NULL)
                return null_node;
            if (current == tree_max())
            {
                return null_node;
            ...
            if (current->right->is_nil)
            {
                ...
                while (current && current->left->is_nil)
        ...
        void    left_rotate(NodePtr x)
            ...
            void init_null_node(NodePtr  node, const value_type &value)
            {
                ...
                node->colour = RED;
                node->is_nil = 0;
                node->value =val_alloc.allocate(1);
            ...
            bool    insert(const value_type &value)
            {
                ...
                while (x && x != null_node)   //Traverse the tree until you reach a null_node pointer
                {
                    ...
                    if (!(_comp(value, *x->value) || _comp(*x->value, value)))
                    {
                        ...
                        val_alloc.deallocate(new_node->value, 1);
            ...
            void clear(NodePtr  node)
            {
                ...
                val_alloc.deallocate(node->value, 1);
                _alloc.destroy(node);
            ...
            void    set_null_node(NodePtr node) 
            {
                null_node = node;
            }
