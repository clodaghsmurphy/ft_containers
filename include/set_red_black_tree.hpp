...
namespace ft{
    ...
    template <typename value_type, typename Compare = ft::less<value_type>, class Allocator = std::allocator<Node<value_type> > >
        class set_rb_tree
        {
            ...
            typedef set_rb_tree<value_type,  Compare, Allocator>     Self ;    
            ...
            set_rb_tree(const Compare comp, const Allocator alloc) : _alloc(alloc), _comp(comp), _size(0)
            {
                std::allocator<value_type> val_alloc;
            ...
            set_rb_tree(const  Self &rhs) : _alloc(rhs._alloc), _comp(rhs._comp), _size(0)
            {
               null_node = _alloc.allocate(1);
                null_node->left = 0;
                null_node->right = 0;
